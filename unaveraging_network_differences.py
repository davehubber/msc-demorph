...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, (images, images_add) in enumerate(test_dataloader):
            images = images.to(device)
            images_add = images_add.to(device)
            sampled_images = diffusion.sample(model, images, images_add, args.sampling_method, epoch, args.run_name, save_image=False)
            images = (images.clamp(-1, 1) + 1) / 2
            images = (images * 255).type(torch.uint8)
            images_add = (images_add.clamp(-1, 1) + 1) / 2
            images_add = (images_add * 255).type(torch.uint8)
            sampled_images.to(device)
            save_images(sampled_images, images, images_add, os.path.join("results", args.run_name, f"{epoch+1}.jpg"))
            break
