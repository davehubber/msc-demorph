...
class Diffusion:
    ...
    def sample(self, model, images, added_images, sampling_method="differences", epoch=0, run_name='flowers_exp2', save_image=True):
        n = len(images)
        model.eval()
        with torch.no_grad():
            ...
            for i in reversed(range(1, self.init_sampling_timestep + 1)):
                ...
                if sampling_method == 'one_step':
                    x = x + model(x, t-1) * t[:, None, None, None]
                    break
                else:
                    x_t = x + model(x, t-1)
                ...
                x = x_t
                if i < self.init_sampling_timestep and i % 60 == 0 or i == 1:
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, (images, images_add) in enumerate(test_dataloader):
            images = images.to(device)
            images_add = images_add.to(device)
            sampled_images, sampled_other_images = diffusion.sample(model, images, images_add, args.sampling_method, epoch, args.run_name, save_image=False)
            images = (images.clamp(-1, 1) + 1) / 2
            images = (images * 255).type(torch.uint8)
            images_add = (images_add.clamp(-1, 1) + 1) / 2
            images_add = (images_add * 255).type(torch.uint8)
            save_images(sampled_images, sampled_other_images, images, images_add, os.path.join("results", args.run_name, f"{epoch+1}.jpg"))
            break
...
def eval(args):
    ...
    print('LPIPS Added: ' + str(np.average(lpips_a2)), flush=True)

def launch():
    import argparse
    parser = argparse.ArgumentParser()
    args = parser.parse_args()
    args.run_name = "flowers"
    args.epochs = 1000
    args.batch_size = 8
    args.image_size = (64, 64)
    args.dataset_path = r"/Oxford102Flowers/jpg"
    args.partition_file = r"csvs/flower_pairs.csv"
    # one_step, differences
    args.sampling_method = r"differences"
    args.sampling_name = r"differences"
